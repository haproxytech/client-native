...
func (x Sites) Diff(y Sites) map[string][]interface{} {
	return DiffSites(x, y)
}

func DiffPointerSite(x, y *Site) map[string][]interface{} {
...
func DiffSites(x, y Sites) map[string][]interface{} {
	diff := make(map[string][]interface{})
	lenX := len(x)
	lenY := len(y)
	...
	for i := 0; i < lenX && i < lenY; i++ {
		...
		for diffKey, diffValue := range DiffPointerSite(vx, vy) {
			diff[key+"."+diffKey] = diffValue
		}
