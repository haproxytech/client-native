...
func TestFilterEqualFalse(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		result.TraceMaxFwd = Ptr(*sample.TraceMaxFwd + 1)
...
func TestFilterDiffFalse(t *testing.T) {
	...
	for _, sample := range samples {
		...
		if len(listDiffFields) != 19 {
			...
			t.Errorf("Expected Filter to be different in 19 cases, but it is not (%d) %s %s", len(result), a, b)
		}
	}
}
