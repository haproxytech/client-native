...
func (rec ClusterSettingsCluster) Equal(obj ClusterSettingsCluster) bool {
	return EqualSlicePointerClusterLogTarget(rec.ClusterLogTargets, obj.ClusterLogTargets) &&
...
func EqualSlicePointerClusterLogTarget(x, y []*ClusterLogTarget) bool {
	...
	for i, vx := range x {
		vy := y[i]
		if !EqualPointerClusterLogTarget(vx, vy) {
			return false
		}
	}
