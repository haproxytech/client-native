...
func TestRuntimeServerEqual(t *testing.T) {
	samples := []struct {
		a, b RuntimeServer
	}{}
	for i := 0; i < 2; i++ {
		var sample RuntimeServer
		var result RuntimeServer
	...
	for _, sample := range samples {
		...
		if !result {
			...
			t.Errorf("Expected RuntimeServer to be equal, but it is not %s %s", a, b)
		}
	}
}

func TestRuntimeServerEqualFalse(t *testing.T) {
	samples := []struct {
		a, b RuntimeServer
	}{}
	for i := 0; i < 2; i++ {
		...
		result.ForcedID = Ptr(*sample.ForcedID + 1)
	...
	for _, sample := range samples {
		...
		if result {
			...
			t.Errorf("Expected RuntimeServer to be different, but it is not %s %s", a, b)
		}
	}
}

func TestRuntimeServerDiff(t *testing.T) {
	samples := []struct {
		a, b RuntimeServer
	}{}
	for i := 0; i < 2; i++ {
	...
	for _, sample := range samples {
		...
		if len(result) != 0 {
			...
			t.Errorf("Expected RuntimeServer to be equal, but it is not %s %s, %v", a, b, result)
		}
	}
}

func TestRuntimeServerDiffFalse(t *testing.T) {
	samples := []struct {
		a, b RuntimeServer
	}{}
	for i := 0; i < 2; i++ {
	...
	for _, sample := range samples {
		...
		if len(listDiffFields) != 26 {
			...
			t.Errorf("Expected RuntimeServer to be different in 26 cases, but it is not (%d) %s %s", len(result), a, b)
		}
	}
}
