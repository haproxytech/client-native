...
func (s *ServiceTestSuit) testCreate() {
	...
	service, err := s.client.NewService(serviceName, configuration.ScalingParams{})
	s.NotNil(service)
	s.Nil(err)
...
func (s *ServiceInitiationSuite) BeforeTest(suiteName, testName string) {
	service, err := s.client.NewService(s.serviceName, configuration.ScalingParams{
		BaseSlots:       baseSlots,
		SlotsGrowthType: configuration.ServiceGrowthTypeLinear,
...
func (s *ServiceInitiationSuite) TestFailCreatingIfExists() {
	_, err := s.client.NewService(s.serviceName, configuration.ScalingParams{})
	s.NotNil(err)
}
...
func (s *ServiceInitiationSuite) TestLoadExistingBackend() {
	...
	s.Nil(err)
	s.Equal(baseSlots, len(s.service.GetNodes()))
...
func (s *ServiceInitiationSuite) createExistingService(servers models.Servers) error {
	...
	for _, server := range servers {
		err := s.client.CreateServer(configuration.BackendParentName, s.serviceName, server, s.transactionID, 0)
		if err != nil {
			return err
		}
	}
	...
	for i := len(servers); i < baseSlots; i++ {
		...
		err := s.client.CreateServer(configuration.BackendParentName, s.serviceName, maintServer, s.transactionID, 0)
		if err != nil {
			return err
		}
	}
...
func (s *ServiceUpdateSuit) validateUpdateResult(expected []configuration.ServiceServer) {
	servers, err := s.service.GetServers()
	s.Nil(err)
...
func (s *ServiceUpdateSuit) TestUpdateScalingParams() {
	newBaseSlots := 30
	err := s.service.UpdateScalingParams(configuration.ScalingParams{
...
func (s *ServiceUpdateSuit) generateServers(count int) []configuration.ServiceServer {
...
func (s *ServiceUpdateSuit) TestExponentialUpscaling() {
	...
	err := s.service.UpdateScalingParams(configuration.ScalingParams{
		BaseSlots:       baseSlots,
		SlotsGrowthType: configuration.ServiceGrowthTypeExponential,
		SlotsIncrement:  slotsIncrement,
	})
	s.Nil(err)
	expected := s.generateServers(baseSlots + 2)
