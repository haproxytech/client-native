...
func resolverExpectation() map[string]models.Resolvers {
	initStructuredExpected()
	res := StructuredToResolverMap()
	// Add individual entries
	for _, vs := range res {
		for _, v := range vs {
			key := v.Name
			res[key] = models.Resolvers{v}
		}
	}
	return res
}
...
func checkResolvers(t *testing.T, got map[string]models.Resolvers) {
