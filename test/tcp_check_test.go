...
// The one from configuration_test.go
func tcpCheckExpectation() map[string]models.TCPChecks {
	initStructuredExpected()
	res := StructuredToTCPCheckMap()
	// Add individual entries
	for k, vs := range res {
		for _, v := range vs {
			key := fmt.Sprintf("%s/%d", k, *v.Index)
			res[key] = models.TCPChecks{v}
		}
	}
	return res
}
...
`
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, checks, err := c.GetTCPChecks(configuration.BackendParentName, "test", "")
			if err != nil {
				t.Error(err.Error())
			}
			...
			_, check, err := c.GetTCPCheck(0, configuration.BackendParentName, "test", "")
			...
			_, check, err = c.GetTCPCheck(counter.increment(), configuration.BackendParentName, "test", "")
...
func checkTCPChecks(t *testing.T, got map[string]models.TCPChecks) {
	exp := tcpCheckExpectation()
	for k, v := range got {
		want, ok := exp[k]
