...
func (c *Client) GetStickResponseRules(backend string, transactionID string) (*models.GetStickResponseRulesOKBody, error) {
	if c.Cache.Enabled() {
		...
		if found {
			return &models.GetStickResponseRulesOKBody{Version: c.Cache.Version.Get(transactionID), Data: stickResRules}, nil
		}
	}
...
func (c *Client) GetStickResponseRule(id int64, backend string, transactionID string) (*models.GetStickResponseRuleOKBody, error) {
	if c.Cache.Enabled() {
		...
		if found {
			return &models.GetStickResponseRuleOKBody{Version: c.Cache.Version.Get(transactionID), Data: stickResRule}, nil
		}
	}
	...
	if c.Cache.Enabled() {
		c.Cache.StickResponseRules.Set(id, backend, transactionID, stickResRule)
	}
